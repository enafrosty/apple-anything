...
export const CenterPreview: React.FC<CenterPreviewProps> = ({ canvasRef }) => {
  ...
  return (
    <div className="flex-1 relative bg-gray-905 flex items-center justify-center p-6 border-b border-gray-900 overflow-hidden">
      {/* GL Canvas */}
      <div className="relative w-full h-full max-w-5xl max-h-[80vh] flex items-center justify-center">
        <canvas
          ref={canvasRef}
          className="w-full h-full max-w-full max-h-full object-contain rounded-lg border border-gray-800 bg-gray-950 shadow-2xl transition-all"
        />

        {/* Status Overlay (e.g. if files are not loaded yet) */}
        {showLoading && (
          <div className="absolute inset-0 bg-gray-950/80 backdrop-blur-sm rounded-lg flex flex-col items-center justify-center gap-4 text-center p-6 select-none border border-gray-800">
        ...
        {store.isExporting && (
          <div className="absolute inset-0 bg-black/90 backdrop-blur-md rounded-lg flex flex-col items-center justify-center gap-5 p-6 border border-gray-855 select-none z-10">
            ...
            <div className="text-center space-y-1.5 max-w-sm w-full">
              <div className="flex justify-between items-center text-xs">
                <span className="text-gray-400 font-semibold uppercase tracking-wider">Exporting...</span>
                <span className="text-purple-400 font-bold">{store.exportProgress}%</span>
              </div>
              ...
              <p className="text-xs text-gray-300 pt-2 font-medium">{store.exportStatusText}</p>
            </div>
          </div>
        )}
      </div>
